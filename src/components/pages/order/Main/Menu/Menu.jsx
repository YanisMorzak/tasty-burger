...
export default function Menu() {
  ...
  const handleCardDelete = (e, idProductToDelete) => {
    ...
    handleDeleteBasketProduct(e, idProductToDelete, username);
    productSelected.id == idProductToDelete
  ...
  if (products.length === 0)
    return isModeAdmin ? (
      <EmptyMenuAdmin resetProducts={() => resetProducts(username)} />
    ) : (
      <EmptyMenuClient />
    );
      ...
      {products.map((produit) => {
        return (
          <Product
            ...
            onDelete={(e) => handleCardDelete(e, produit.id)}
            onClick={() => handleClick(produit.id)}
            isHoverable={isModeAdmin}
            isSelected={
              isModeAdmin &&
              checkedIfProductIsClicked(produit.id, productSelected.id)
            }
            onAdd={(e) => handleAddButton(e, produit.id)}
          />
        );
      })}
    </MenuStyled>
